...
<?php
$enable_newsxo_typo = get_theme_mod('enable_newsxo_typo', false);
if ($enable_newsxo_typo == true) { ?>
<style>
/* Headings Typography*/
h1, .h1, h2, .h2, h3, .h3, h4, .h4, h5, .h5, h6, .h6, .wp-block-search__label  {
  font-weight:<?php echo newsxo_get_option('heading_fontweight'); ?> !important;
  font-family:<?php echo newsxo_get_option('heading_fontfamily'); ?> !important;
}
/* Menus Font Family*/
.navbar-wp .sm-clean > li > a, .navbar-wp .dropdown-menu > li > a{ 
  font-family:<?php echo esc_attr(get_theme_mod('newsxo_menu_fontfamily','Inter'))?> !important; 
}
</style>
<?php }
}